...
> 详细部署和使用教程见：[docs/DEPLOY_AND_USAGE.md](docs/DEPLOY_AND_USAGE.md)
...
本分支默认不再内置弱口令。`WEB_PASS`、`PROXY_PASS`、`AGENT_TOKEN` 必须通过 Cloudflare Wrangler Secrets 配置；安装 Agent 的 `/agent` 与 `/scripts/*` 接口也必须携带 `AGENT_TOKEN` 才能访问。
...
2. 创建 D1 数据库，并记录输出里的 `database_id`：
...
脚本会逐步询问 Worker 名称、D1 数据库、面板用户名、代理用户名；`WEB_PASS`、`PROXY_PASS`、`AGENT_TOKEN` 可以直接回车自动生成。脚本会更新 `wrangler.toml`，并在 `.deploy/` 下生成本地 secrets 文件和上传脚本。`.deploy/` 已加入 `.gitignore`，不要提交到 GitHub。
...
5. 登录 Worker 面板后，复制右上角带 `AGENT_TOKEN` 的 VPS 纳管命令到 VPS 上执行。

> 建议在 VPS 防火墙只放行你的客户端 IP 访问代理端口，不要把代理端口长期暴露给全网。
...
4. **配置环境变量 (Environment Variables)**：本分支无安全默认密码；密码与 token 必须使用 Wrangler Secrets 或 Cloudflare 控制台 Secret 变量配置。

| 变量名 | 推荐配置方式 | 作用说明 |
| --- | --- | --- |
| `WEB_USER` | 普通变量 | 面板 Web 登录用户名 |
| `WEB_PASS` | Secret | 面板 Web 登录密码 |
| `PROXY_USER` | 普通变量 | Socks5/HTTP 代理连接用户名 |
| `PROXY_PASS` | Secret | Socks5/HTTP 代理连接密码 |
| `AGENT_TOKEN` | Secret | VPS Agent 注册、下载脚本与心跳认证 token |
...
```bash
bash <(curl -fsSL "https://您的worker域名.workers.dev/agent?token=您的AGENT_TOKEN")
